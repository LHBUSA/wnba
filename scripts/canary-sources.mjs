...
const winba = await get(`${API}/v1/stats/winba`);
check('WinBA Score', winba, (b) => ({ pass: b.ok && b.data.version === 'winba/1.0.1' && b.data.status === 'AVAILABLE' && b.data.qualified_count > 0 && b.data.rows.some((r) => r.qualified && r.rank === 1), detail: `${b.data.qualified_count || 0} qualified · ${b.data.provisional_count || 0} provisional · ${b.data.games_used || 0} finals` }));
