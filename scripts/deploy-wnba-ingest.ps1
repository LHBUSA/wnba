...
Write-Host 'Running PBE runner/runtime tests...' -ForegroundColor Cyan
node --test tests/pbe-runner.test.mjs tests/pbe-runtime.test.mjs
if ($LASTEXITCODE -ne 0) { throw 'PBE tests failed. Refusing production deploy.' }
...
if ($health.version -ne '1.0.4') { throw "Unexpected deployed version: $($health.version)" }
...
if ($health.pbe_release -ne 'pbe-lock-grade-recovery-2026-09-19') { throw "Unexpected PBE release marker: $($health.pbe_release)" }
