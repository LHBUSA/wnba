...
export const description = () => 'WNBA Pro: PBE Picks, Edge Timeline, Player Load, Rotation Impact, Scenario Lab, Watchlist, model reasoning and the live locked track record. $9.99/month or $3.99/week.';
...
export async function mount(root, ctx) {
  ...
  const checkout = () => {
    ...
    return checkoutReady(plan, account)
      ...
      : html`<button class="btn gold block" style="margin-top:16px" type="button" disabled aria-disabled="true">Founding Season checkout opens soon</button><p class="note" style="margin-top:10px;text-align:center">Purchases are not open yet. Nothing is charged from this page until secure Stripe checkout is live.</p>`;
  };
  const signInBlock = () => html`<div style="border-top:1px solid var(--line);margin-top:18px;padding-top:14px"><p class="note"><b>Already a member, or want to sign in first?</b> Enter your email and we’ll send a one-time sign-in link. No password.</p>${signInReady ? html`<form class="signin-form" data-signin novalidate><input type="email" name="email" autocomplete="email" inputmode="email" placeholder="you@example.com" aria-label="Email address" required ${signin.status === 'sending' ? 'disabled' : ''} /><button class="btn" type="submit" ${signin.status === 'sending' ? 'disabled' : ''}>${signin.status === 'sending' ? 'Sending…' : 'Email me a link'}</button></form>${signin.message ? html`<p class="signin-msg ${signin.status === 'error' ? 'err' : ''}" role="status">${signin.message}</p>` : ''}` : html`<p class="note" style="margin-top:8px">Member sign-in opens with WNBA Pro checkout.</p>`}</div>`;
  ...
  const proPanel = () => html`<section class="card pro-panel desk-active">
    ...
    ${account.access === 'subscriber' ? html`<dl class="kv" style="margin-top:18px"><dt>Plan</dt><dd>${PLANS[account.plan]?.label || '—'} · ${PLANS[account.plan]?.price || ''}/${PLANS[account.plan]?.per || ''}</dd><dt>Status</dt><dd>${account.status}${account.cancel_at_period_end ? ' · ends at period end' : ''}</dd><dt>${account.cancel_at_period_end ? 'Access until' : 'Renews'}</dt><dd>${fmtDateET(account.current_period_end, { month: 'long', day: 'numeric', year: 'numeric' })}</dd></dl>` : ''}
    ...
    <button class="btn block" style="margin-top:16px" type="button" data-logout>Sign out</button><p class="note" style="margin-top:14px;text-align:center">Billing is managed by Stripe. Manage or cancel from your Stripe receipt email.</p>
  </section>`;

  const draw = () => {
    const s = account.state;
    render(root, html`${preview ? html`<div class="callout warn" style="margin-bottom:16px">DESIGN PREVIEW · dev build only · state “${preview}” with specimen values. Production renders only the state the server returns.</div>` : ''}<div class="pro-wrap"><section class="pro-hero"><span class="eyebrow">Founding Season</span><h1 style="margin-top:14px">WNBA <span>Pro</span></h1><p style="margin-top:14px;color:var(--paper-2);font-size:16px;max-width:58ch">PBE Picks is now the center of a full intelligence stack: model movement, player workload, rotation pressure, scenario paths, live watchlists and a permanent locked-call record.</p>${valueList(FLAGSHIP)}<span class="eyebrow" style="display:block;margin-top:22px">The full WNBA desk</span>${valueList(DESK)}<div class="pill-row" style="margin-top:18px"><a class="pill on" href="/brief">Try the free Daily Brief</a>${PRO_INTELLIGENCE.map((f) => html`<a class="pill" href="${f.href}">${f.name}</a>`)}</div>${s !== 'pro' ? html`<a class="btn gold" style="margin-top:22px" href="#plans">Unlock WNBA Pro</a>` : ''}<p class="note" style="margin-top:18px">Player Load measures workload and schedule pressure, not medical fatigue. Scenario Lab v1 explains evidence paths rather than inventing simulations. Model details: <a class="sec-link" href="/pbe-picks/model">how the PBE model works</a> · <a class="sec-link" href="/track-record">live track record</a>.</p></section>${s === 'pro' ? proPanel() : html`<section class="card pro-panel" id="plans"><div class="pro-state">${s === 'free' ? html`${badge('final', 'Signed in')}<span>${account.email}</span>` : html`${badge('sched', 'Choose your plan')}`}</div><h2 style="font:800 30px/1 var(--f-display);text-transform:uppercase;margin:16px 0">${s === 'free' ? 'Upgrade to WNBA Pro' : 'Unlock WNBA Pro'}</h2>${planPicker()}${checkout()}<ul class="note" style="margin:16px 0 0;padding-left:18px;display:grid;gap:4px"><li>No free trial</li><li>Cancel anytime</li><li>WNBA Pro is separate from NBA, NHL and UFC Pro</li></ul>${s === 'free' ? html`${account.entitlement_check === 'UNAVAILABLE' ? html`<p class="callout warn" style="margin-top:14px">We couldn’t confirm your subscription just now. If you’re a member, reload in a minute.</p>` : ''}<button class="btn" style="margin-top:14px" type="button" data-logout>Sign out</button>` : signInBlock()}</section>`}</div>`);
