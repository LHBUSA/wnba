...
export const description = () => 'WNBA Pro: PBE Picks, Player Load Intelligence, model reasoning, live track record, WNBACast, Best Line, matchups and availability. $9.99/month or $3.99/week.';

const FLAGSHIP = [
  ['PBE Picks', 'Independent win probabilities, market comparison and PBE Edge.'],
  ['Player Load Intelligence', 'A 0–100 workload and schedule-pressure index using recent minutes, game density, turnaround, overtime and rotation context.'],
...
export async function mount(root, ctx) {
  ...
  const draw = () => {
    ...
    render(root, html`${preview ? html`<div class="callout warn" style="margin-bottom:16px">DESIGN PREVIEW · dev build only · state “${preview}” with specimen values. Production renders only the state the server returns.</div>` : ''}<div class="pro-wrap"><section class="pro-hero"><span class="eyebrow">Founding Season</span><h1 style="margin-top:14px">WNBA <span>Pro</span></h1><p style="margin-top:14px;color:var(--paper-2);font-size:16px;max-width:52ch">PBE Picks plus proprietary Player Load Intelligence: the model view and the player workload view, built as separate research signals.</p>${valueList(FLAGSHIP)}<span class="eyebrow" style="display:block;margin-top:22px">The full WNBA desk</span>${valueList(DESK)}${s !== 'pro' ? html`<a class="btn gold" style="margin-top:22px" href="#plans">Unlock WNBA Pro</a>` : ''}<p class="note" style="margin-top:18px">Player Load measures workload and schedule pressure, not medical fatigue. Model details: <a class="sec-link" href="/pbe-picks/model">how the PBE model works</a> · <a class="sec-link" href="/track-record">live track record</a>.</p></section>${s === 'pro' ? proPanel() : html`<section class="card pro-panel" id="plans"><div class="pro-state">${s === 'free' ? html`${badge('final', 'Signed in')}<span>${account.email}</span>` : html`${badge('sched', 'Choose your plan')}`}</div><h2 style="font:800 30px/1 var(--f-display);text-transform:uppercase;margin:16px 0">${s === 'free' ? 'Upgrade to WNBA Pro' : 'Unlock WNBA Pro'}</h2>${planPicker()}${checkout()}<ul class="note" style="margin:16px 0 0;padding-left:18px;display:grid;gap:4px"><li>No free trial</li><li>Cancel anytime</li><li>WNBA Pro is separate from NBA, NHL and UFC Pro</li></ul>${s === 'free' ? html`${account.entitlement_check === 'UNAVAILABLE' ? html`<p class="callout warn" style="margin-top:14px">We couldn’t confirm your subscription just now. If you’re a member, reload in a minute.</p>` : ''}<button class="btn" style="margin-top:14px" type="button" data-logout>Sign out</button>` : signInBlock()}</section>`}</div>`);
