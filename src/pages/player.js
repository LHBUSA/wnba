...
export async function mount(root, ctx) {
  ...
  ctx.setMeta({ title: `${p.name}${p.team ? ` · ${p.team.name}` : ''}`, description: `${p.name} (${p.team?.name || 'WNBA'}): season and recent stats, game log, minutes trend, availability and WNBA news.` });
    ...
    <section class="p-hero">
      ...
      <div style="display:flex;flex-direction:column;justify-content:flex-end;min-width:0">
        ...
        <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ${inj ? html`${statusBadge(inj.status)}<span class="note">${[inj.side, inj.body_part].filter(Boolean).join(' ')} · ESPN feed updated ${fmtDateET(inj.source_updated_at, { month: 'short', day: 'numeric' })}</span>` : d.availability ? badge('final', 'Not on injury feed') : badge('stale', 'Availability unknown')}
        </div>
    ...
    <div class="section split">
      <section class="card">
        ...
        <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Date</th><th>Opp</th><th>Result</th><th>MIN</th><th>PTS</th><th>REB</th><th>AST</th><th>FG</th><th>3PT</th><th>TO</th><th></th></tr></thead><tbody>
          ${games.slice(0, 25).map((g) => html`<tr><td>${fmtDateET(g.date, { month: 'short', day: 'numeric' })}</td><td class="l">${g.at_vs === '@' ? '@' : 'vs'} ${g.opponent?.abbr || ''}</td><td class="l">${g.result || ''} ${g.score || ''}</td><td>${num(g.min, 0)}</td><td class="hi">${g.pts ?? '—'}</td><td>${g.reb ?? '—'}</td><td>${g.ast ?? '—'}</td><td>${g.fgm ?? '—'}-${g.fga ?? '—'}</td><td>${g.fg3m ?? '—'}-${g.fg3a ?? '—'}</td><td>${g.tov ?? '—'}</td><td><a class="gold" href="/cast/${g.game_id}">Replay</a></td></tr>`)}
        </tbody></table></div>
      ...
      <aside class="grid" style="gap:16px;align-content:start">
        <section class="card">
          <div class="card-head"><span class="card-title">Props</span></div>
          <div class="card-body">
