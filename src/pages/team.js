...
import { sourceLine, errorState, skeleton, playerCard, gameCard, statusBadge, avatar, badge } from '../ui/components.js';
...
export async function mount(root, ctx) {
  ...
  const d = res.data;
  const t = d.team;
  ctx.setMeta({ title: t.name, description: `${t.name}: WNBA roster, schedule and lines, observed rotation, season profile, availability and PropBetEdge newsroom coverage.` });
  ...
  render(root, html`
    <section class="team-hero" style="--tc1:${col.color || '#d4af37'};--tc2:${col.alt || '#ff7a2f'}">
      ...
      <div class="team-hero-in">
        <span class="kicker" style="font:700 11px/1 var(--f-data);letter-spacing:.24em;text-transform:uppercase;color:var(--gold)">${d.season?.label || ''}${st ? ` · ${st.conference_name}` : ''}</span>
    ...
    <div class="section split">
      <div>
        ...
        <section class="section">
          <div class="sec-head"><h2 class="sec-title bc">Season profile</h2><span class="note">per game · ESPN team totals</span></div>
          <div class="tiles">
