...
async function detail(root, ctx) {
  ...
  const [res, arts] = await Promise.all([api.matchup(ctx.params.gameId), api.articles({ game: ctx.params.gameId, limit: 6 })]);
  if (!ctx.isCurrent()) return;
    ...
    <section class="team-hero" style="--tc1:${teamColors(g.away).color || '#8fb4ff'};--tc2:${teamColors(g.home).color || '#d4af37'};margin-bottom:16px">
      <div style="padding:clamp(18px,3vw,34px)">
        ...
        <div class="mu-head" style="margin-top:16px">
          <div class="mu-team" style="display:flex;gap:14px;align-items:center">${teamLogo(g.away, 72)}<span><b>${g.away?.name}</b><small>${g.away?.record || ''} · away</small></span></div>
          <span class="mu-vs">AT</span>
          <div class="mu-team home" style="display:flex;gap:14px;align-items:center;justify-content:flex-end"><span><b>${g.home?.name}</b><small>${g.home?.record || ''} · home${g.venue?.name ? ` · ${g.venue.name}` : ''}</small></span>${teamLogo(g.home, 72)}</div>
        </div>
        <div class="pill-row" style="margin-top:16px"><a class="pill on" href="/cast/${g.game_id}">WNBACast</a><a class="pill" href="/props">Best line board</a>${d.season_series?.[0]?.summary ? html`<span class="chip">${d.season_series[0].summary}</span>` : ''}</div>
      </div>
    </section>
