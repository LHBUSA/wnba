    ...
    <div class="grid g2">
      ${d.groups.map((g) => html`<section class="card">
        ...
        <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Team</th><th>W</th><th>L</th><th>PCT</th><th>GB</th><th>L10</th><th>STRK</th><th>HOME</th><th>ROAD</th><th>DIFF</th></tr></thead><tbody>
          ${g.entries.map((t) => html`<tr><td><a class="pname" href="/teams/${t.team_id}"><span class="mono faint" style="width:16px">${t.seed ?? ''}</span>${t.name}${t.clincher ? html`<span class="clinch" title="ESPN clinch mark">${t.clincher}</span>` : ''}</a></td><td>${t.wins}</td><td>${t.losses}</td><td>${num(t.win_pct, 3).replace(/^0/, '')}</td><td>${t.games_behind}</td><td>${t.last_ten || '—'}</td><td>${t.streak || '—'}</td><td>${t.home || '—'}</td><td>${t.road || '—'}</td><td>${signed(t.differential)}</td></tr>`)}
        </tbody></table></div>
      </section>`)}
    </div>
